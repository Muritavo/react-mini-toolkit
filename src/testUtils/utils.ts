...
export function getDiagnosticsForText(files: { [filename: string]: string }) {
  const fileNames = Object.keys(files);
  const asts = fileNames.reduce(
    (r, filename) => ({
      ...r,
      [filename]: ts.createSourceFile(
        filename,
        files[filename],
  ...
  const host: ts.CompilerHost = {
    fileExists: (filePath) =>
      fileIsMocked(cleanFilePath(filePath)) ||
      nativeHost.fileExists(
        workaroundForPathBecauseIDnotKnowTSCompilersYet(filePath)
      ),
    directoryExists: (dirPath) =>
      dirPath === "/root/path/to/project/src/folder/MockComponent" ||
      nativeHost.directoryExists!(
        workaroundForPathBecauseIDnotKnowTSCompilersYet(dirPath)
      ),
    getCurrentDirectory: () => "/root/path/to/project/src/folder/MockComponent",
    ...
    getSourceFile: (filePath) =>
      fileIsMocked(cleanFilePath(filePath))
        ? asts[cleanFilePath(filePath)]
        : nativeHost.getSourceFile(
            workaroundForPathBecauseIDnotKnowTSCompilersYet(filePath),
            ts.ScriptTarget.Latest
          ),
    readFile: (filePath) =>
      fileIsMocked(cleanFilePath(filePath))
        ? files[cleanFilePath(filePath)]
        : nativeHost.readFile(
            workaroundForPathBecauseIDnotKnowTSCompilersYet(filePath)
          ),
